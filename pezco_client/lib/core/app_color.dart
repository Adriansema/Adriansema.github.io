import 'package:flutter/material.dart';

class AppColors {
  static const Color primary = Color(0xFF29237E);
  static const Color secondary = Color(0xFF32B5AE);
  static const Color accent = Color(0xFF3B32B5);
  static const Color elements = Color(0xFFdddddd);

  //Backgrond
  static const Color background = Color(0xFFF4F4F4);
  static const Color backgroundComponent = Color(0x08000000);
  static const Color backgroundComponentSelected = Color(0x80ffffff);
  static const Color backgroundComponentMenu = Color(0x4Dffffff);

}